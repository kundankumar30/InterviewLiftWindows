  ...
  try {
    ...
    if (vadResult.hasVoice) {
      ...
      if (vadResult.bufferedChunks && vadResult.bufferedChunks.length > 0) {
        console.log(
          `🎤 VAD: Voice activity started - sending ${vadResult.bufferedChunks.length} buffered chunks`
        );
        for (const bufferedChunk of vadResult.bufferedChunks) {
          if (!(bufferedChunk instanceof Buffer)) {
            console.error(
              "Invalid buffered chunk: not a Buffer",
              bufferedChunk
            );
          ...
          console.log(
            `🎤 VAD: Processing buffered chunk (size: ${bufferedChunk.length} bytes)`
          );
          const googleSTTResult =
            googleSpeechTranscriber.processAudioChunk(bufferedChunk);
          console.log(
            `🎤 VAD: Processed buffered chunk with Google STT, result:`,
            googleSTTResult
          );
          if (!googleSTTResult || !googleSTTResult.success) {
            console.warn("Failed to process buffered chunk:", googleSTTResult);
            vadStats.discardedChunks++;
          }
        }
      }
      console.log(
        `🎤 VAD: Processing current chunk (size: ${audioDataChunk.length} bytes)`
      );
      const googleSTTResult =
        googleSpeechTranscriber.processAudioChunk(audioDataChunk);
      console.log(
        `🎤 VAD: Processed current chunk with Google STT, result:`,
        googleSTTResult
      );
      if (!googleSTTResult || !googleSTTResult.success) {
